 ...
 * checkbox action, deduped by a stable actionKey.
 */
...
function parseInboxPayloads(markdown, inputPath, args) {
  ...
  for (const rawLine of markdown.split(/\r?\n/)) {
    ...
    const object = parseObjectKey(line);
    if (object) {
      section.object = object;
      continue;
    }

    section.firstSeenDate = section.firstSeenDate || parseSectionDate(line, 'First seen');
    section.lastSeenDate = section.lastSeenDate || parseSectionDate(line, 'Last seen');
    section.sourceDate = section.sourceDate || parseSectionDate(line, 'Source date');
    ...
    if (!inActionsSection || !section.object) {
      continue;
    }
    ...
    const sourceMarkdownSection = truncate([
      section.heading,
...
function summarizePayload(payload) {
  ...
  return `${target} ${payload.sourceDate} ${payload.status} - ${payload.actionText}`;
}
...
async function main() {
  ...
  if (payloads.length === 0) {
    throw new Error('No inbox payloads found in the markdown input.');
  }
