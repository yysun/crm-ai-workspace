...
- Both posting scripts must be dry-run first when payload shape or route behavior changed. Actual writes require the script-specific environment gate: `AIW_ENABLE_CRM_INBOX_POST=1` for `Inbox`, `AIW_ENABLE_CRM_ACTION_POST=1` for explicit `Actions` archives.
