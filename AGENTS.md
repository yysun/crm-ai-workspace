...
- `scripts/split-agent-authored-summaries.js`: guarded splitter for completed agent-authored batch files. It validates complete `summary-target` Markdown blocks and writes them byte-for-byte to sibling `*-summary.md` paths; it must not synthesize, rewrite, fill, or improve judgment text.
...
- `scripts/build-accumulated-actions.js`: creates deterministic team-scoped dated accumulated-action queue snapshots and removed-action files under `data/{teamId}/daily-triage/{yyyy}/{mm}/{dd}/` from in-place `summary.md` artifacts, including added, carried, and removed actions from `## Proposed Actions`. Snapshot action rows expose business fields such as team ID, account/contact identity, action text, action title, action category, first/last seen dates, source date, and latest summary path; internal normalized action keys, stale summary paths, and status fields are not output fields.
- `scripts/post-accumulated-actions.js`: gated CRM write helper for explicitly archiving one accumulated daily markdown report per team/date to the CRM `Actions` table through `POST /api/data/actions`. This is for snapshot/audit text only, not default daily operations and not inbox work items. It requires `AIW_ENABLE_CRM_ACTION_POST=1` unless `--dry-run` is used.
- `scripts/post-inbox.js`: gated CRM write helper for upserting checkbox-level inbox work items and same-day removals to the CRM `Inbox` table through `POST /api/data/inbox`. It parses account/contact checkbox actions from `actions-{yyyy-mm-dd}.md`, reads matching `removed-actions-{yyyy-mm-dd}.json` when present, derives `actionKey`, includes `actionTitle`, `actionCategory`, full `actionText`, evidence-only trace content, and requires `AIW_ENABLE_CRM_INBOX_POST=1` unless `--dry-run` is used.
- `scripts/post-inbox-sql.js`: gated direct SQL Inbox writer that reuses the same checkbox payload parser, marks selected-team open SQL rows stale when their `ActionKey` is no longer in the current local queue, and merges current active/done/removed rows into `dbo.Inbox` by `(TeamId, ActionKey)` with `ActionTitle`, `ActionCategory`, full action text, trace, and judgment fields. It reads `SQL_SERVER`, `SQL_DATABASE`, `SQL_USER`, and `SQL_PASSWORD` from `.env` and requires `AIW_ENABLE_SQL_INBOX_UPSERT=1` unless `--dry-run` is used.
- `scripts/batch-remediate-summaries.js`: creates fixed Codex CLI remediation manifests and worker prompt files for agent-authored `*-summary.md` refreshes. It may launch workers with GPT-5.5 medium by default, but it must not draft, split, or rewrite summary judgment itself.
- `scripts/run-daily-inbox-pipeline.js`: top-level coordinator for refresh/local-only daily Inbox runs. It may chain deterministic scripts, Codex CLI worker batches, validation, accumulated-action rebuild, index rebuild, Inbox dry-run, and gated Inbox post; it must not author `summary.md` content or post CRM `Actions` archives by default.
...
- `process/daily-process.md`: full daily publish-run contract for the operator phrase `download data, distill today, post inbox` and legacy wording such as `post actions and post inbox`; coordinates refresh, agent-authored distillation, accumulated-action rebuild, enriched Inbox dry-run validation, and gated Inbox posting without scripting the judgment layer. CRM `Actions` posting is archive-only and must be explicitly requested.
...
- `scripts/post-inbox.js` is the default daily operational publisher. It posts checkbox-level work items and same-day removed action states to the CRM `Inbox` table only. Use it when the user asks to upsert inbox items, publish inbox work, send checkbox actions to the inbox, or run the normal daily process. The default date is today; `--date=YYYY-MM-DD` selects another daily triage date across all available team files. For inbox rows, map workspace team `0` or CRM team `-1` to API team `8`. Use `scripts/post-inbox-sql.js` only when the publish target is direct SQL; select teams explicitly when cleanup should not touch every available team queue for the date.
- `scripts/post-accumulated-actions.js` posts accumulated daily markdown snapshots to the CRM `Actions` table only when the user explicitly asks to archive accumulated action reports, action markdown, or daily action snapshots. For team-scoped daily reports, use `--team-file`; `data/0/...` maps to API team `8` in this snapshot route when that API expects the CRM team identifier. This script is not for inbox upserts and is not the daily operating queue.
- Do not confuse the two destinations: normalized checkbox work items and removal states with `actionKey` belong in `Inbox`; accumulated markdown snapshots belong in `Actions` only as explicit archives.
- Both posting scripts must be dry-run first when payload shape or route behavior changed. Actual writes require the script-specific environment gate: `AIW_ENABLE_CRM_INBOX_POST=1` for API `Inbox`, `AIW_ENABLE_SQL_INBOX_UPSERT=1` for direct SQL `Inbox`, and `AIW_ENABLE_CRM_ACTION_POST=1` for explicit `Actions` archives.
