...
- `scripts/post-inbox.js`: gated CRM write helper for upserting checkbox-level inbox work items and same-day removals to the CRM `Inbox` table through `POST /api/data/inbox`. It parses account/contact checkbox actions from `actions-{yyyy-mm-dd}.md`, reads matching `removed-actions-{yyyy-mm-dd}.json` when present, derives `actionKey`, includes trace content, and requires `AIW_ENABLE_CRM_INBOX_POST=1` unless `--dry-run` is used.
- `scripts/post-inbox-sql.js`: gated direct SQL Inbox writer that reuses the same checkbox payload parser, marks selected-team open SQL rows stale when their `ActionKey` is no longer in the current local queue, and merges current active/done/removed rows into `dbo.Inbox` by `(TeamId, ActionKey)`. It reads `SQL_SERVER`, `SQL_DATABASE`, `SQL_USER`, and `SQL_PASSWORD` from `.env` and requires `AIW_ENABLE_SQL_INBOX_UPSERT=1` unless `--dry-run` is used.
